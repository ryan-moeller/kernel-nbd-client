    ...
    size_t *lengthp)
{
	...
	offset = reply_offset_data.offset;
    ...
    struct nbd_structured_reply *reply, struct bio *bp)
{
	...
	offset = reply_offset_hole.offset;
...
nbd_conn_recv(struct nbd_conn *nc)
{
	...
	if (reply.magic == NBD_STRUCTURED_REPLY_MAGIC) {
		...
		/* TODO: The length should be allowed fixed header overhead. */
		if (__predict_false(length > nc->nc_softc->sc_maxpayload)) {
			G_NBD_LOGREQ(G_NBD_ERROR, bp,
			    "%s invalid structured reply length (%zu)",
			    __func__, length);
			nbd_conn_degrade_state(nc, NBD_CONN_HARD_DISCONNECTING);
