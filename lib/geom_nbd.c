    ...
    struct nbd_export_info *info)
{
	...
	if (((info->transmission_flags >> 16) & NBD_FLAG_NO_ZEROES) != 0)
		return (0);
	return (nbd_client_recv(client, info->reserved, sizeof info->reserved));
}
...
nbd_connect(struct gctl_req *req, unsigned flags)
{
	...
	gctl_ro_param(req, "minimum_blocksize", sizeof client.minimum_blocksize,
	    &client.minimum_blocksize);
